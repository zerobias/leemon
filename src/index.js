...
function millerRabinInt(x: number[], b: number) {
...
function millerRabin(x: number[], b: number[]) {
  var i, j, k, s;
...
function addInt(x: number[], n: number) {
...
export function powMod(x: number[], y: number[], n: number[]) {
...
function inverseMod(x: number[], n: number[]) {
  var ans=expand(x, n.length);
  var s;
  s=inverseMod_(ans, n);
...
function multMod(x: number[], y: number[], n: number[]) {
  var ans=expand(x, n.length);
  multMod_(ans, y, n);
  return trim(ans, 1);
}
...
function randTruePrime_(ans: number[], k: number) {
...
function randBigInt_(b: number[], n: number, s: number) {
  var i, a;
...
//y is destroyed.
function GCD_(x: number[], y: number[]) {
...
function inverseMod_(x: number[], n: number[]) {
...
export function eGCD_(x: number[], y: number[], v: number[], a: number[], b: number[]) {
  var g=0;
...
function greaterShift(x: number[], y: number[], shift: number) {
...
export function greater(x: number[], y: number[]) {
  var i;
...
export function divide_(x: number[], y: number[], q: number[], r: number[]) {
  var kx, ky;
  var i, j, y1, y2, c, a, b;
  copy_(r, x);
...
function modInt(x: number[], n: number) {
  var i, c=0;
...
export function str2bigInt(s: string, base: number, minSize?: number): number[] {
  var d, i, x, y, kk;
  var k=s.length;
...
export function rightShift_(x: number[], n: number) {
  var i;
...
export function leftShift_(x: number[], n: number) {
  var i;
  var k=Math.floor(n/bpe);
  if (k) {
...
//do x=x*n where x is a bigInt and n is an integer.
//x must be large enough to hold the result.
function multInt_(x: number[], n: number) {
  var i, k, c, b;
  if (!n)
    return;
  k=x.length;
...
function divInt_(x: number[], n: number) {
  var i, r=0, s;
  for (i=x.length-1; i>=0; i--) {
    s=r*radix+x[i];
    x[i]=Math.floor(s/n);
...
function linComb_(x: number[], y: number[], a: number, b: number) {
  var i, c, k, kk;
...
//x must be large enough to hold the answer.
function linCombShift_(x: number[], y: number[], b: number, ys: number) {
  var i, c, k, kk;
...
//x must be large enough to hold the answer.
function addShift_(x: number[], y: number[], ys: number) {
  var i, c, k, kk;
  k=x.length<ys+y.length ? x.length : ys+y.length;
  kk=x.length;
  for (c=0, i=ys; i<k; i++) {
...
//do x=x-(y<<(ys*bpe)) for bigInts x and y, and integer ys.
//x must be large enough to hold the answer.
function subShift_(x: number[], y: number[], ys: number) {
  var i, c, k, kk;
  k=x.length<ys+y.length ? x.length : ys+y.length;
  kk=x.length;
  for (c=0, i=ys; i<k; i++) {
...
export function sub_(x: number[], y: number[]) {
  var i, c, k, kk;
  k=x.length<y.length ? x.length : y.length;
  for (c=0, i=0; i<k; i++) {
    c+=x[i]-y[i];
...
//do x=x+y for bigInts x and y.
//x must be large enough to hold the answer.
export function add_(x: number[], y: number[]) {
  var i, c, k, kk;
  k=x.length<y.length ? x.length : y.length;
  for (c=0, i=0; i<k; i++) {
    c+=x[i]+y[i];
...
function mult_(x: number[], y: number[]) {
  var i;
...
function mod_(x: number[], n: number[]) {
...
function multMod_(x: number[], y: number[], n: number[]) {
  var i;
...
function squareMod_(x: number[], n: number[]) {
  var i, j, d, c, kx, kn, k;
...
function trim(x: number[], k: number) {
  var i, y;
...
function powMod_(x: number[], y: number[], n: number[]) {
  var k1, k2, kn, np;
...
//  n is odd
//  np = -(n^(-1)) mod radix
function mont_(x: number[], y: number[], n: number[], np: number) {
  var i, j, c, ui, t, ks;
  var kn=n.length;
  var ky=y.length;
